...
void ProbeRenderer::createRenderPipeline() {
  ...
  const auto writeSets =
      std::vector{uniformDebugWrite, computeDebugProbesWrite, computeProbesWrite, computeSmallProbesWrite,
                  gridInfoWrite,     cameraDebugWrite,        proxGridWrite,      proxGridInfoWrite};
  (*vkLogicalDevice)->updateDescriptorSets(writeSets, nullptr);
