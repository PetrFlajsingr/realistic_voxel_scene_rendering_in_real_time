...
class ColorChooser : public LabeledElement,
 ...
 protected:
  void unserialize_impl(const toml::table &src) override {
    OneOf<glm::vec3, glm::vec4> auto color = T{};
    const auto tomlColor = src["color"].as_array();
    for (auto i : std::views::iota(0, T::length())) {
      color[i] = **tomlColor->get(i)->as_floating_point();
    }
  ...
  toml::table serialize_impl() override {
    ...
    auto tomlColor = toml::array{};
    for (auto i : std::views::iota(0, T::length())) { tomlColor.push_back(color[i]); }
