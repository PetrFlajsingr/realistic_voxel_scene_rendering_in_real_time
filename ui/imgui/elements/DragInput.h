...
class DragInput : public ValueObservableElement<T>, public LabeledElement {
 public:
  ...
  DragInput(const std::string &elementName, const std::string &caption, ParamType speed,
      ...
      : Element(elementName), ValueObservableElement<T>(elementName, value),
        LabeledElement(elementName, caption), speed(speed), min(min), max(max),
 ...
 protected:
  void renderImpl() override {
    ...
    if constexpr (std::same_as<T, float>) {
      ImGui::DragFloat(getLabel().c_str(), ValueObservableElement<T>::getValueAddress(),
                       speed, min, max, format.c_str());
    }
    if constexpr (std::same_as<T, glm::vec2>) {
      ImGui::DragFloat2(getLabel().c_str(),
                        glm::value_ptr(*ValueObservableElement<T>::getValueAddress()), speed,
    ...
    if constexpr (std::same_as<T, int>) {
      ImGui::DragInt(getLabel().c_str(), ValueObservableElement<T>::getValueAddress(), speed,
                     min, max, format.c_str());
    }
    ...
    if constexpr (std::same_as<T, math::Range<int>>) {
      ImGui::DragIntRange2(
          getLabel().c_str(),
          reinterpret_cast<int *>(ValueObservableElement<T>::getValueAddress()), speed, min,
          max, format.c_str());
    }
