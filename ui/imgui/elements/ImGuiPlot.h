//
// Created by petr on 11/1/20.
//

#ifndef REALISTIC_VOXEL_RENDERING_UI_IMGUI_ELEMENTS_IMGUIPLOT_H
#define REALISTIC_VOXEL_RENDERING_UI_IMGUI_ELEMENTS_IMGUIPLOT_H

#include "interface/ImGuiCaptionedElement.h"
#include <optional>
#include <vector>

namespace pf::ui {

enum class PlotType { Lines, Histogram };

class ImGuiPlot : public ImGuiCaptionedElement {
 public:
  ImGuiPlot(const std::string &elementName, const std::string &caption, PlotType plotType,
            std::vector<float> values = {}, std::optional<std::string> overlayText = std::nullopt,
            const std::optional<std::size_t> &historyLimit = std::nullopt);
  void addValue(float value);
  void clear();
  void setValues(const std::vector<float> &vals);

 protected:
  void renderImpl() override;

 private:
  PlotType plotType;
  std::vector<float> values;
  std::optional<std::string> overlayText = std::nullopt;
  std::optional<std::size_t> historyLimit = std::nullopt;
};
}// namespace pf::ui
#endif//REALISTIC_VOXEL_RENDERING_UI_IMGUI_ELEMENTS_IMGUIPLOT_H
