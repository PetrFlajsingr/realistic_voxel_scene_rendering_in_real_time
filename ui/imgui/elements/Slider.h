...
template<OneOf<IMGUI_SLIDER_TYPE_LIST> T>
class Slider : public LabeledElement, public ValueObservableElement<T> {
 public:
  using MinMaxType = details::SliderMinMaxType<T>;
  Slider(const std::string &elementName, const std::string &caption, MinMaxType min,
              MinMaxType max, T value = T{},
              std::string format = details::defaultSliderFormat<MinMaxType>())
      : Element(elementName),
        LabeledElement(elementName, caption), ValueObservableElement<T>(elementName,
                                                                                    value),
 ...
 protected:
  void renderImpl() override {
    ...
    if constexpr (std::same_as<T, float>) {
      ImGui::SliderFloat(getLabel().c_str(), ValueObservableElement<T>::getValueAddress(),
                         min, max, format.c_str());
    }
    if constexpr (std::same_as<T, glm::vec2>) {
      ImGui::SliderFloat2(getLabel().c_str(),
                          glm::value_ptr(*ValueObservableElement<T>::getValueAddress()), min,
                          max, format.c_str());
    }
    ...
    if constexpr (std::same_as<T, int>) {
      ImGui::SliderInt(getLabel().c_str(), ValueObservableElement<T>::getValueAddress(), min,
                       max, format.c_str());
    }
